...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        double averageTaskProceedTime = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(days -> (days.getDeadline().toEpochDay() - days.getCreated().toEpochDay()))
                .map(longObj -> LocalDate.ofEpochDay(longObj).getDayOfYear())
                .mapToDouble(x -> x)
                .average().getAsDouble();

        double expected = (double) (16 + 36 + 6) / 3;

        //Then
        assertEquals(expected, averageTaskProceedTime);
    }
}